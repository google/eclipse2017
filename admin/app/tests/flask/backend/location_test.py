...
class LocationTests(unittest2.TestCase):
    """
    ...
    def _get_location_root(self, headers=None, expected_status=200):
        if headers is None: headers = self.HEADERS
        ret_val = self.test_client.get('/services/admin/users/locations/',
                                       headers=headers)
    ...
    def test_get_location_missing_auth(self):
        self._get_location_root(headers={}, expected_status=405)
